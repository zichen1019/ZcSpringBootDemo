...
public class Agent {
    ...
    private static byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) {
        if (className == null || "null".equals(className) || "".equals(className)) return new byte[0];
        System.err.println("ClassName：" + className);
        if ("org/apache/ibatis/logging/jdbc/BaseJdbcLogger".equals(className)) {
            try {
        ...
        } else if ("com/alibaba/druid/pool/DruidPooledPreparedStatement$PreparedStatementKey".equals(className)) {
            try {
                ClassPool pool = ClassPool.getDefault();
                CtClass ctc = pool.get(className.replace("/", "."));
                CtConstructor[] ctcons = ctc.getDeclaredConstructors();
